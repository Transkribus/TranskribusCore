...
public class HtrUtils {
	...
	//this file stores relative (to lineImgDir i.e. workdir) paths to feature files
	...
	public static final String FILENAME_PATTERN = "([A-Z]{24})" + SEP_PATTERN + "([0-9]+)" + SEP_PATTERN + "([0-9]+)" + SEP_PATTERN + "([a-zA-Z0-9[_-]]+)" + SEP_PATTERN + "([a-zA-Z0-9[_-]]+)";
	
	private static final String	FLOAT_NR = "[+-]?(?:[0-9]*\\.[0-9]+|[0-9]+)"; // regex for floating point nr. with optional - sign
//	private static final String LATTICE_TOOL_LINE_PATTERN_STR = "-?[0-9]+\\.[0-9]+\\s-?[0-9]+\\.[0-9]+\\s[0-9]+\\s<s>\\s(.*)\\s</s>";
	 ...
	 * positional number of region and line according to the reading order, 
	 * and the original IDs of region and line appearing in the PAGE file.
	 * 
	 * TODO implement mapping of name schema in both directions, e.g. for mapping the results back to entities					
	 */	
	...
	public static String buildBaseFileName(final String pageFileId, final int regIndex, final int lineIndex, 
		...
		if(regId.contains(SEP) || lineId.contains(SEP)){
			throw new IllegalArgumentException("A regionID contains the illegal character \""+ SEP + "\":" + regId + " | " + lineId);
		}
...
//				logger.debug(tiStr + tfStr + word + confStr);
				WordHypothesis hyp = new WordHypothesis(Integer.parseInt(tiStr), Integer.parseInt(tfStr), word, Double.parseDouble(confStr));
				wordList.add(hyp);
			} else {
